...
- Direct Atlassian URL preview and registration are local-only operations that resolve or create the normalized-domain Site and Item/Space without any Source Instance, Provider, reference, capability, or remote lookup. Separate access setup is also local-only: it binds a registered Site to an official-Atlassian or Gateway Source Instance only after the user supplies the Provider and an actual shape-validated Cloud ID or Gateway configuration alias. Provider is never inferred from URL content; credentials and tokens are never accepted. Key-only text, invalid URLs, invalid references, and failed access setup create no partial rows and trigger no remote lookup.
