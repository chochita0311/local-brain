...
Atlassian owns separate Jira and Confluence inventories. Browse is the default mode; setup and URL addition remain an explicit neighboring mode. A ticket, Page, project, or Space starts from a real URL; key-only text is not treated as evidence. The URL is parsed locally into service, Site domain, kind, and available identifier. An existing unambiguous Source Instance/Site is reused, while a first domain requires one explicit local access-path choice—official Atlassian MCP or company MCP Gateway—before Source Instance, Site, and reference are created atomically. Provider describes that stable LocalBrain access path and is never inferred from the URL alone. The connection may remain local-only and unbound until a validated Cloud ID or Gateway alias is supplied. Direct URL setup and registration perform no remote read and do not imply remote validation. Jira projects default to selected content, while regular Confluence Spaces default to full Page content. Discovering accessible Space candidates is a visibly separate, bounded maintenance action whose result may be partial and requires confirmation before registration.
