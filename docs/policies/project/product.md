...
- A persisted primary Session detail also projects bounded `Related Context` from existing local data. One target is deduplicated and explains every applicable relationship in this priority order: Session-backed Atlassian evidence, shared Thread, shared Workstream, then an enabled Local Context Document in the same workspace. The projection returns at most 12 entries after scanning at most 48 candidates from each source family, orders equal-strength entries by title, type, and stable ID, and never substitutes global recency. Missing local paths and unsafe external destinations remain visible but inactive; unresolved polymorphic targets are omitted with a bounded count. Loading the projection copies no content body, writes no relationship, and starts no model, embedding, remote, capability, scan, or maintenance work. A projection error never blocks the conversation. At widths through `920px`, the rail follows Session identity and orientation in document order and precedes the conversation; Subsession detail has no rail.
...
Sessions Dashboard defaults to Daily with the latest 30 inclusive local-calendar days. Weekly shows the current and previous 11 Monday-based weeks, while Cumulative starts at the earliest eligible real-model usage date and presents monthly running totals. Source, range, Tokens/Cost, and paired custom dates are reproducible GET state. The four-metric overview reports estimated cost, total normalized tokens, usage-linked primary-work Sessions, and active days; only the Session denominator excludes maintenance and subsession records. MTD values and data freshness remain subordinate context rather than a second KPI wall.
...
One composition family explains the selected records by Source, normalized Model, or first-observation Project snapshot. Shares use only the compatible selected token or priced-cost denominator, while unsupported values remain unavailable. Raw model identity remains inspectable, Project attribution never changes through a current-path lookup, and `Unassigned` stays visible. A separate trust region owns source synchronization, retained stale/error data, token and price coverage, calculation state, and price-snapshot explanation.
